...
trait MyPromise[T]
  extends mycon.MyFuture[T] with mycon.MyPromise[T] {
  def future = this

  import mycon.impl.MyPromise.DefaultPromise
}
  ...
  class DefaultPromise[T](val name: String = "default") extends AtomicReference[AnyRef](Nil) with MyPromise[T] {
    ...
    override def onComplete[U](f: Try[T] => U, name: String = "default"): Unit = {
      ...
      get match {
        ...
        case list: Seq[CallbackRunnable[T]] =>
          logger.debug(s"${this.name} hasn't finished. so $name's callback is added to ${this.name}")
    ...
    override def transform[S](f: Try[T] => Try[S], name: String = "default"): MyFuture[S] = {
      ...
      onComplete(
        ...
        s"${this.name}'s-callback-for-$name"
      )
      ...
      onComplete(
        { r1 =>
          f(r1).onComplete(
            ...
            s"${this.name}'s-inner-callback-for-$name"
          )
        },
        s"${this.name}'s-outer-callback-for-$name"
      )
      // 新しく作成した
      promise.future
    }
